...
SETTING = utils.BoolSetting(
    SETTING_NAME, "Whether or not to leave inactive channels after 2 weeks"
)
...
class Module(ModuleManager.BaseModule):
    ...
    def _set_timestamp(self, channel):
        channel.set_setting(
            "last-message", utils.datetime.format.iso8601(utils.datetime.utcnow())
        )
    ...
    def hourly(self, event):
        ...
        for server in self.bot.servers.values():
            serverwide_setting = server.get_setting(SETTING_NAME, botwide_setting)
            if not serverwide_setting:
                continue

            mode_setting = server.get_setting(MODE_SETTING_NAME, botwide_mode_setting)
        ...
        for server, channel in parts:
            log.warn(
                "Leaving %s:%s due to channel inactivity", [str(server), str(channel)]
            )
