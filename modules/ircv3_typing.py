...
class Module(ModuleManager.BaseModule):
    def _tagmsg(self, target, state):
        return IRCLine.ParsedLine("TAGMSG", [target], tags={"+draft/typing": state})
    ...
    def preprocess(self, event):
        if self._has_tags(event["server"]) and self._expect_output(event):
            ...
            event["server"].send(
                self._tagmsg(event["target_str"], "active"), immediate=True
            )
        else:
    ...
    def postprocess(self, event):
        if (
            event["target"]._typing
            and not event["stdout"].has_text()
            and not event["stderr"].has_text()
        ):
