...
class Module(ModuleManager.BaseModule):
    ...
    def process(self, service):
        ...
        for k, a in times.items():
            if not a["orig"]:
                continue
            a["short"] = (
                a["datetime"].strftime("%H%M") if len(a["orig"]) > 5 else a["orig"]
            )
            a["shortest"] = (
                "%02d" % a["datetime"].minute
                if -300 < a["ut"] - ut_now < 1800
                else a["short"]
            )
            ...
            if "a" + k[1:] in service:
                a["status"] = {"d": 0, "a": 3}[k[2]]
            if k[0] == "s":
                a["status"] = 4

        arr, dep = [times[a] for a in a_types if times[a]["ut"]], [
            times[a] for a in d_types if times[a]["ut"]
        ]
    ...
    def activities(self, string):
        return [
            a + b.strip() for a, b in list(zip(*[iter(string)] * 2)) if (a + b).strip()
        ]
    ...
    def trains(self, event):
        ...
        filter = self.filter(
            " ".join(event["args_split"][1:]) if len(event["args_split"]) > 1 else "",
            {
                ...
                "power": (
                    None,
                    lambda x: x.upper() in ["EMU", "DMU", "HST", "D", "E", "DEM"],
                    lambda x: x.upper(),
                ),
        ...
        if filter["inter"] and filter["type"] != "departure":
            raise utils.EventError(
                "Filtering by intermediate stations is only "
                "supported for departures."
            )

        nr_filterlist = client.factory.create("filterList")
        if filter["inter"]:
            nr_filterlist.crs.append(filter["inter"])
        ...
        method = (
            client.service.GetArrivalDepartureBoardByCRS
            if len(location_code) == 3
            else client.service.GetArrivalDepartureBoardByTIPLOC
        )
        try:
            query = method(
                100,
        ...
        nrcc_severe = (
            len([a for a in query["nrccMessages"][0] if a["severity"] == "Major"])
            if "nrccMessages" in query
            else 0
        )
        if event.get("external"):
            ...
            if nrcc_severe:
                severe_summary += ", "
                severe_summary += utils.irc.bold(
                    utils.irc.color(
                        "%s severe messages" % nrcc_severe, utils.consts.RED
                    )
                )
            station_summary = "%s (%s, %s%s)" % (
                query["locationName"],
        ...
        if (
            not "trainServices" in query
            and not "busServices" in query
            and not "ferryServices" in query
        ):
        ...
        trains = []
        services = []
        if "trainServices" in query:
            services += query["trainServices"][0]
        if "busServices" in query:
            services += query["busServices"][0]
        if "ferryServices" in query:
        ...
        for t in services:
            parsed = {
                ...
                "delayed": t["departureType"] == "Delayed"
                if "departureType" in t
                else None,
                "cancel_reason": t["cancelReason"]["value"]
                if "cancelReason" in t
                else "",
            ...
            parsed["departure_only"] = location_code in [
                a["code"] for a in parsed["origins"]
            ]
            ...
            if parsed["cancelled"] or parsed["delayed"]:
                for k, time in parsed["times"].items():
                    time["short"], time["on_time"], time["status"], time["prefix"] = (
                        "%s:%s"
                        % (
                            "C" if parsed["cancel_reason"] else "D",
        ...
        for t in trains:
            t["dest_summary"] = "/".join(
                [
                    "%s%s"
                    % (
                        a["code"] * filter["crs"] or a["name"],
                        " " + a["via"] if a["via"] else "",
                    )
                    for a in t["destinations"]
                ]
            )
            t["origin_summary"] = "/".join(
                [
                    "%s%s"
                    % (
                        a["code"] * filter["crs"] or a["name"],
        ...
        trains = sorted(
            trains,
            key=lambda t: t["times"]["max_sched"]["ut"]
            if filter["type"] == "both"
            else t["times"]["st" + filter["type"][0]]["ut"],
        )
        ...
        for train in trains:
            if not True in [
                (train["destinations"], train["toc"]) in train_locs_toc
                and (filter["dedup"] or filter["default"]),
                filter["dest"]
                and not filter["dest"].upper()
                in [a["code"] for a in train["destinations"]],
                filter["origin"]
                and not filter["origin"].upper()
                in [a["code"] for a in train["origins"]],
                ...
                filter["power"]
                and not filter["power"] == train.get("power_type", None),
            ]:
        ...
        if event.get("external"):
            trains_string = "\n".join(
                [
                    ...
                    % (
                        ...
                        t["origin_summary"]
                        if t["terminating"] or filter["type"] == "arrival"
                        else t["dest_summary"],
                    )
            ...
            trains_string = ", ".join(
                [
                    ...
                    % (
                        "from "
                        if not filter["type"][0] in "ad" and t["terminating"]
                        else "",
                        t["origin_summary"]
                        if t["terminating"] or filter["type"] == "arrival"
                        else t["dest_summary"],
                        ...
                        "?"
                        if "platformsAreUnreliable" in query
                        and query["platformsAreUnreliable"]
                        else "",
                        t["times"][filter["type"]]["prefix"].replace(
                            filter["type"][0], ""
                        )
                        if not t["cancelled"]
                        else "",
        ...
        if event.get("external"):
            ...
            event["stdout"].write(
                ...
                % (
                    station_summary,
                    " departures calling at %s" % filter["inter"]
                    if filter["inter"]
                    else "",
                    trains_string,
                )
    ...
    def service(self, event):
        ...
        rid = service_id
        if len(service_id) <= 8:
            query = client.service.QueryServices(
                service_id,
            ...
            if query and len(query["serviceList"][0]) > 1:
                return event["stdout"].write(
                    ...
                    + ", ".join(
                        [
                            "%s (%s->%s)"
                            % (a["uid"], a["originCrs"], a["destinationCrs"])
                            for a in query["serviceList"][0]
                        ]
                    )
                )
            if query:
            ...
            if schedule:
                ...
                stype = (
                    "%s %s"
                    % (schedule_query.data["tops_inferred"], schedule["power_type"])
                    if schedule_query.data["tops_inferred"]
                    else schedule["power_type"]
                )
                for k, v in {
                    "operatorCode": schedule["atoc_code"],
                    "serviceType": stype
                    if stype
                    else SCHEDULE_STATUS[schedule["status"]],
                }.items():
                    query[k] = v
        ...
        if "cancelReason" in query:
            disruptions.append(
                ...
                % (
                    ...
                    " at " + query["cancelReason"]["_tiploc"]
                    if query["cancelReason"]["_tiploc"]
                    else "",
                )
            )
        if "delayReason" in query:
            disruptions.append(
                ...
                % (
                    ...
                    " at " + query["delayReason"]["_tiploc"]
                    if query["delayReason"]["_tiploc"]
                    else "",
                )
            )
        if disruptions and not external:
            disruptions = (
                utils.irc.color(", ".join(disruptions), utils.consts.RED) + " "
            )
        ...
        stations = []
        for station in (
            query["locations"][0] if "locations" in query else schedule["locations"]
        ):
            if "locations" in query:
                parsed = {
                    "name": station["locationName"],
                    "crs": (
                        station["crs"] if "crs" in station else station["tiploc"]
                    ).rstrip(),
                    ...
                    "last": False,
                    "cancelled": station["isCancelled"]
                    if "isCancelled" in station
                    else False,
                    "associations": [],
                    ...
                    "activity": self.activities(station["activities"])
                    if "activities" in station
                    else [],
                    "activity_p": self.reduced_activities(station["activities"])
                    if "activities" in station
                    else [],
                }
                ...
                associations = (
                    station["associations"][0] if "associations" in station else []
                )
                for assoc in associations:
                    parsed_assoc = {
                        "uid_assoc": assoc.uid,
                        "category": {"divide": "VV", "join": "JJ", "next": "NP"}[
                            assoc["category"]
                        ],
                        "from": parsed["first"],
                        ...
                        "origin_crs": assoc["originCRS"]
                        if "originCRS" in assoc
                        else None,
        ...
        for station in stations:
            for assoc in station["associations"]:
                assoc[
                    "summary"
                ] = "{arrow} {assoc[category]} {assoc[uid_assoc]} {dir_arrow} {assoc[far_name]} ({code})".format(
                    assoc=assoc,
                    arrow=assoc["from"] * "<-" or "->",
            ...
            if station["passing"]:
                (
                    ...
                    station["times"]["departure"]["status"],
                ) = (5, 5)
            elif station["called"]:
                (
                    station["times"]["arrival"]["status"],
            ...
            station["summary"] = "%s%s (%s%s%s%s%s)%s" % (
                ...
                station["length"] + " car, "
                if station["length"] and (station["first"] or station["associations"])
                else "",
                ...
                ", ".join(
                    [a["summary"] for a in station["associations"]]
                    if filter["associations"]
                    else ""
                ),
            )
            station["summary_external"] = "%1s%-5s %1s%-5s %-3s %-3s %-3s %s%s" % (
                "~" * station["times"]["a"]["estimate"]
                + "s" * (station["times"]["a"]["schedule"]),
                station["times"]["a"]["short"],
                "~" * station["times"]["d"]["estimate"]
                + "s" * (station["times"]["d"]["schedule"]),
                ...
                if station["associations"]
                else "",
            )

        stations_filtered = []
        for station in stations:
            if station["passing"] and not filter["passing"]:
                continue
    ...
    def service_code(self, event):
        ...
        if not event["args"].isnumeric():
            raise utils.EventError("The delay/cancellation code must be a " "number")
        reasons = {
            a["code"]: (a["lateReason"], a["cancReason"])
            for a in client.service.GetReasonCodeList()[0]
        }
