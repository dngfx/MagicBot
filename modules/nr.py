...
class Module(ModuleManager.BaseModule):
        ...
        for a in a_types + d_types:
            if a in service and service[a]:
                ...
                if len(service[a]) > 5:
                    ...
                    times[a]["datetime"] = datetime.strptime(datetime.now().date().isoformat() + "T" + service[a][:4],
                                                             "%Y-%m-%dT%H%M")
    ...
    def trains(self, event):
        ...
        if filter["inter"] and filter["type"] != "departure":
            raise utils.EventError("Filtering by intermediate stations is only " "supported for departures.")
        ...
        nrcc_severe = len([a for a in query["nrccMessages"][0] if a["severity"] == "Major"
                           ]) if "nrccMessages" in query else 0
        if event.get("external"):
            ...
            station_summary = "%s (%s, %s%s)" % (query["locationName"],
                                                 query["crs"],
                                                 query["stationManagerCode"],
                                                 severe_summary)

        if not "trainServices" in query and not "busServices" in query and not "ferryServices" in query:
            return event["stdout"].write("%s: No services for the next %s minutes" % (station_summary,
                                                                                      filter["period"]))
        ...
        for t in trains:
            t["dest_summary"] = "/".join([
                "%s%s" % (a["code"] * filter["crs"] or a["name"],
                          " " + a["via"] if a["via"] else '') for a in t["destinations"]
            ])
            t["origin_summary"] = "/".join([
                "%s%s" % (a["code"] * filter["crs"] or a["name"],
                          " " + a["via"] if a["via"] else '') for a in t["origins"]
            ])

        trains = sorted(trains,
                        key=lambda t: t["times"]["max_sched"]["ut"]
                        if filter["type"] == "both" else t["times"]["st" + filter["type"][0]]["ut"])
    ...
    def service(self, event):
        ...
        if "cancelReason" in query:
            disruptions.append("Cancelled (%s%s)" %
                               (query["cancelReason"]["value"],
                                " at " + query["cancelReason"]["_tiploc"] if query["cancelReason"]["_tiploc"] else ""))
        if "delayReason" in query:
            disruptions.append("Delayed (%s%s)" %
                               (query["delayReason"]["value"],
                                " at " + query["delayReason"]["_tiploc"] if query["delayReason"]["_tiploc"] else ""))
        ...
        for station in stations:
            ...
            station["summary"] = "%s%s (%s%s%s%s%s)%s" % (
                ...
                station["times"][filter["type"]]["prefix"].replace(filter["type"][0],
                                                                   ""),
                utils.irc.color(station["times"][filter["type"]]["short"],
        ...
        if external:
            ...
            event["stdout"].write("%s%s %s %s (%s/%s): %s" % (disruptions,
                                                              query["operatorCode"],
                                                              query["trainid"],
                                                              query["serviceType"],
                                                              done_count,
    ...
    def head(self, event):
        ...
        if not query:
            raise utils.EventError("No currently running services match this " "identifier")
        ...
        if event.get("external"):
            ...
            event["stdout"].write(", ".join([
                "h/%s r/%s u/%s rs/%s %s (%s) -> %s (%s)" % (a["trainid"],
                                                             a["rid"],
                                                             a["uid"],
                                                             a["rsid"],
                                                             a["originName"],
    ...
    def service_code(self, event):
        ...
        reasons = {a["code"]: (a["lateReason"],
                               a["cancReason"]) for a in client.service.GetReasonCodeList()[0]}
