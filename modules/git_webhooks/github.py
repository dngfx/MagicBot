...
class GitHub(object):
    ...
    def push(self, full_name, data):
        ...
        return self._format_push(
            branch, author, data["commits"], data["forced"], single_url, range_url
        )
        ...
        if action == "opened":
            ...
            if commits:
                ...
                if new_commits:
                    ...
                    outputs = self._format_push(
                        pr_identifier, author, new_commits, False, single_url, range_url
                    )
    ...
    def pull_request_review(self, full_name, data):
        ...
        return [
            "[PR] %s %s on %s: %s - %s" % (reviewer, state_desc, number, pr_title, url)
        ]
    ...
    def fork(self, full_name, data):
        ...
        fork_full_name = utils.irc.color(
            data["forkee"]["full_name"], utils.consts.LIGHTBLUE
        )
        url = self._short_url(data["forkee"]["html_url"])
        return ["%s forked into %s - %s" % (forker, fork_full_name, url)]
