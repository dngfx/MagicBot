...
class Module(ModuleManager.BaseModule):
    ...
    def roulette(self, event):
        ...
        winnings_str = [
            "%s for %s (%d to 1)"
            % (winnings[bet][1] * winnings[bet][0], bet, winnings[bet][0])
            for bet in winnings.keys()
        ]
        ...
        if not losses and winnings:
            ...
            event["stdout"].write(
                "Roulette spin lands on %s, "
                "%s loses %s"
                % (choice, event["user"].nickname, self._coin_str_human(coin_losses))
            )
    ...
    def jackpot(self, event):
        ...
        event["stdout"].write(
            "%s: The current jackpot is %s"
            % (event["user"].nickname, (LOTTERY_BUYIN * count))
        )
    ...
    def next_lottery(self, event):
        ...
        event["stdout"].write(
            "Next lottery is in: %s" % utils.datetime.format.to_pretty_until(until)
        )
    ...
    def _lottery(self, event):
        for server in self.bot.servers.values():
            ...
            winnings = LOTTERY_BUYIN * len(users)
