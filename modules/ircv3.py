...
class Module(ModuleManager.BaseModule):
            ...
            if server.get_setting("ircv3-stats", False):
                if spec in server.server_capabilities:
                    ...
                    if server.connection_params.tls:
                        port = "+%s" % port
                    hostname = utils.irc.bold("%s:%s" % (server.connection_params.hostname, port))

                    supporting_servers.append("%s (%s %s)" % (str(server), hostname, server.version))
    ...
    def capabilities(self, event):
        event["stdout"].write("IRCv3 capabilities: %s" % ", ".join(event["server"].agreed_capabilities))
