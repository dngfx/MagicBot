...
class Module(ModuleManager.BaseModule):
    ...
    def eval(self, event):
        ...
        try:
            page = utils.http.request(EVAL_URL, post_data=args, method="POST", content_type="application/json").json()
        except socket.timeout:
            raise utils.EventError("%s: eval timed out" % event["user"].nickname)
    ...
    def crate(self, event):
        ...
        if response.code == 200:
            ...
            name = crate["id"]
            url = URL_CRATE % name
            event["stdout"].write("%s %s: %s - %s" % (name, crate["max_version"], crate["description"], url))
        else:
