...
@utils.export(
    "channelset",
    utils.BoolSetting(
        "highlight-spam-protection", "Enable/Disable highlight spam protection"
    ),
)
@utils.export(
    "channelset",
...
class Module(ModuleManager.BaseModule):
    ...
    def highlight_spam(self, event):
        if event["channel"].get_setting("highlight-spam-protection", False):
            ...
            if len(highlights) > 1 and len(highlights) >= event["channel"].get_setting(
                ...
                if not has_mode:
                    if should_ban:
                        event["channel"].send_ban(
                            "*!%s@%s" % (event["user"].username, event["user"].hostname)
                        )
                    event["channel"].send_kick(
                        event["user"].nickname, "highlight spam detected"
                    )
