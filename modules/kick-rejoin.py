...
delay_setting = utils.IntSetting(
    "kick-rejoin-delay", "Amount of seconds to wait before rejoining a channel"
)
...
class Module(ModuleManager.BaseModule):
    def _should_rejoin(self, server, channel):
        return channel.get_setting(
            "kick-rejoin", server.get_setting("kick-rejoin", False)
        )

    def _get_delay(self, server, channel):
        return channel.get_setting(
            "kick-rejoin-delay", server.get_setting("kick-rejoin-delay", DELAY)
        )
