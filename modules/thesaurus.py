...
class Module(ModuleManager.BaseModule):
    ...
    def thesaurus(self, event):
        ...
        page = utils.http.request(URL_THESAURUS % (self.bot.config["bighugethesaurus-api-key"], phrase))
        ...
        if page:
            ...
            if not len(event["args_split"]) > 1:
                ...
                if word_types:
                    ...
                    event["stdout"].write("Available categories for %s: %s" % (phrase, ", ".join(word_types)))
                else:
                ...
                if category in page:
                    if syn_ant in page[category]:
                        event["stdout"].write("%ss for %s: %s" % (event["command"].title(),
                                                                  phrase,
                                                                  ", ".join(page[category][syn_ant])))
                    else:
                        event["stderr"].write("No %ss for %s" % (event["command"], phrase))
                else:
