...
ABAN_METHODS = {"chary": "$a:", "insp": "R:", "unreal": "~a:"}
...
KICK_REASON_SETTING = utils.Setting(
    "default-kick-reason", "Set the default kick reason", example="have a nice trip"
)
...
@utils.export(
    "channelset",
    utils.Setting(
        "ban-format", "Set ban format (%s)" % BAN_FORMATTING, example="*!${u}@${h}"
    ),
)
@utils.export(
    "channelset",
...
class Module(ModuleManager.BaseModule):
    ...
    def flags(self, event):
        target = event["spec"][1]
        current_flags = event["spec"][0].get_user_setting(target.get_id(), "flags", "")

        if not event["spec"][2]:
            current_flags_str = ("+%s" % current_flags) if current_flags else ""
            event["stdout"].write("Flags for %s: %s" % (target, current_flags_str))
        else:
            ...
            if new_flags:
                ...
                new_flags = sorted(
                    new_flags, key=lambda c: ("0" if c.islower() else "1") + c
                )

                new_flags_str = "".join(new_flags)
                event["spec"][0].set_user_setting(
                    target.get_id(), "flags", new_flags_str
                )

                self._check_flags(event["server"], event["spec"][0], target)

                event["stdout"].write(
                    "Set flags for %s to +%s" % (target.nickname, new_flags_str)
                )
            else:
    ...
    def _check_flags(self, server, channel, user):
        ...
        if channel.get_setting("auto-voice", False):
            avmode = []
            current_modes = channel.get_user_modes(user)

            if not "v" in current_modes:
                avmode.append(("v", user.nickname))
                channel.send_modes(avmode, True)
    ...
    def cmute(self, event):
        ...
        if event["spec"][1]:
            self.timers.add_persistent(
                "unmode", event["spec"][1], channel=event["spec"][0].id, mode="m"
            )
    ...
    def clear(self, event):
        mode, prefix = self._type_to_mode(
            event["server"], event["spec"][0], event["spec"][1]
        )

        mode_list = self._list_query_event(
            event["spec"][0], event["spec"][2], mode, prefix
        )
    ...
    def lsearch(self, event):
        ...
        if mode_list:
            event["stdout"].write(
                "%s: %s" % (event["user"].nickname, " ".join(mode_list))
            )
        else:
    ...
    def _find_mode(self, type, server):
        if type == "ban":
            ...
            if not "INVEX" in server.isupport:
                raise utils.EventError("invexes are not supported on this network")
    ...
    def kick(self, event):
        self._mask_kick(
            event["server"], event["spec"][0], event["spec"][1], event["spec"][2]
        )
    ...
    def kickban(self, event):
        ...
        self._mask_kick(
            event["server"], event["spec"][0], event["spec"][2], event["spec"][3]
        )
    ...
    def _mask_mode(self, server, user, spec, type):
        ...
        if users:
            if target_type == TargetType.MASK:
                ...
                args = [u.nickname for u in users if not spec[0].has_mode(u, mode)]
        ...
        if args:
            ...
            if not spec[1] == None:
                self.timers.add_persistent(
                    "unmode", spec[1], channel=spec[0].id, args=args
                )
    ...
    def mask_unmode(self, event):
        ...
        if event["spec"][1][0] == "user":
            ...
            masks = self._list_query_event(
                event["spec"][0], event["spec"][1][1], mode, prefix
            )
    ...
    def access_unmode(self, event):
        ...
        _, mode, _ = self._find_mode(event["hook"].get_kwarg("type"), event["server"])
        valid_nicks = [u.nickname for u in users if event["spec"][0].has_mode(u, mode)]
