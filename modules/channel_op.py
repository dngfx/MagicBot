...
KICK_REASON_SETTING = utils.Setting("default-kick-reason", "Set the default kick reason", example="have a nice trip")
...
class Module(ModuleManager.BaseModule):
    ...
    def flags(self, event):
        ...
        if not event["spec"][2]:
            ...
            if new_flags:
                ...
                new_flags = sorted(new_flags, key=lambda c: ("0" if c.islower() else "1") + c)

                new_flags_str = "".join(new_flags)
                event["spec"][0].set_user_setting(target.get_id(), "flags", new_flags_str)

                self._check_flags(event["server"], event["spec"][0], target)

                event["stdout"].write("Set flags for %s to +%s" % (target.nickname, new_flags_str))
            else:
    ...
    def clear(self, event):
        mode, prefix = self._type_to_mode(event["server"], event["spec"][0], event["spec"][1])
    ...
    def mask_unmode(self, event):
        is_mask, mode, prefix = self._find_mode(event["hook"].get_kwarg("type"), event["server"])

        users = args = []
        if event["spec"][1][0] == "user":
            masks = [self._get_hostmask(event["spec"][0], event["spec"][1][1])]
        elif event["spec"][1][0] == "word":
            masks = self._list_query_event(event["spec"][0], event["spec"][1][1], mode, prefix)
    ...
    def down(self, event):
        if not event["spec"][1] or event["spec"][1][1] == event["user"]:
            ...
            event["check_assert"](utils.Check("channel-mode", "o") | utils.Check("channel-access", "high,down"))
