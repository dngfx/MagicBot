...
class Module(ModuleManager.BaseModule):
    ...
    def _uptime(self):
        return utils.datetime.format.to_pretty_since(int(time.time() - self.bot.start_time))
    ...
    def _stats(self):
        ...
        for server in self.bot.servers.values():
            networks[server.alias.lower()] = [len(server.channels), len(server.users)]
    ...
    def stats(self, event):
        ...
        if event["args"]:
            ...
            event["stdout"].write("On %s, I have %d %s and %d visible %s" % (alias,
                                                                             channels,
                                                                             self._plural("channel",
                                                                                          channels),
                                                                             users,
                                                                             self._plural("user",
                                                                                          users)))
            ...
            event["stdout"].write("I currently have %d %s, %d %s and %d visible %s" % (network_count,
                                                                                       network_plural,
                                                                                       total_channels,
                                                                                       channel_plural,
                                                                                       total_users,
    ...
    def _server_stats(self, server):
        return {
            ...
            "channels": {c.name: self._channel_stats(c) for c in server.channels},
    ...
    def channels_api(self, event):
        if event["args"]:
            ...
            for server in self.bot.servers.values():
                channels[server.id] = {}
                for channel in server.channels.values():
                    channels[server.id][str(channel)] = self._channel_stats(channel)
            return channels

    @utils.hook("received.command.channels")
    @utils.kwarg("help", "List all the channel I'm in on this network (* = hidden)")
    ...
    def channels_command(self, event):
        ...
        event["stdout"].write("Current channels: %s" % " ".join(sorted(channels)))
