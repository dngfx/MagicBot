...
class Module(ModuleManager.BaseModule):
    ...
    def my_id(self, event):
        ...
        """
        event["stdout"].write("%s: %d" % (event["user"].nickname, event["user"].get_id()))

    @utils.hook("received.command.myaccount")
    @utils.kwarg("help", "Show what I think your account name is")
    def account(self, event):
        event["stdout"].write("%s: %s" % (event["user"].nickname, self.exports.get_one("account-name")(event["user"])))
    ...
    def channel_id(self, event):
        ...
        """
        event["stdout"].write("%s: %d" % (event["target"].name, event["target"].id))
