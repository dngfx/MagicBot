    ...
    def hash(self, event):
        ...
        if event["args_split"]:
            ...
            if algorithm in hashlib.algorithms_available:
                cipher_text = hashlib.new(algorithm, phrase.encode("utf8")).hexdigest()
                event["stdout"].write("(%s) %s -> %s" % (utils.irc.bold(algorithm), phrase, cipher_text))
            else:
                event["stderr"].write("Unknown algorithm provided")
        else:
            event["stdout"].write("Available algorithms: %s" % ", ".join(hashlib.algorithms_available))
