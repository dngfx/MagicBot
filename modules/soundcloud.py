...
class Module(ModuleManager.BaseModule):
    ...
    def soundcloud(self, event):
        ...
        page = utils.http.request(URL_SOUNDCLOUD_TRACK if has_query else URL_SOUNDCLOUD_RESOLVE,
        ...
        if page:
            if len(page):
                ...
                duration = time.strftime("%H:%M:%S", time.gmtime(page["duration"] / 1000))
                if duration.startswith("00:"):
                    duration = duration[3:]
                link = page["permalink_url"]
                event["stdout"].write("%s [%s] (posted by %s) %s" % (title, duration, user, link))
            else:
