...
class Module(ModuleManager.BaseModule):
    ...
    def _channel_message(self, user, event):
        ...
        self._set_words_date(event["user"], event["channel"], date, user_words + word_count)
    ...
    def tracked_words(self, event):
        event["stdout"].write("Tracked words: %s" % ", ".join(event["server"].get_setting("tracked-words", [])))
    ...
    def wordiest(self, event):
        ...
        if event["spec"][0]:
            ...
            date2 = utils.datetime.format.date_human(event["spec"][1] or utils.datetime.utcnow())
