...
def iso8601(dt: _datetime.datetime, timespec: TimeSpec = TimeSpec.NORMAL) -> str:
...
def to_pretty_since(total_seconds: int, max_units: int = UNIT_MINIMUM) -> str:
    return to_pretty_time(total_seconds, max_units, RelativeDirection.BACKWARD)


def to_pretty_until(total_seconds: int, max_units: int = UNIT_MINIMUM) -> str:
