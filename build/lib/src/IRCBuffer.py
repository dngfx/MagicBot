...
class BufferLine(object):
    ...
    notes: typing.Dict[str, str] = dataclasses.field(default_factory=dict)
...
class Buffer(object):
    ...
    def get(
        self, index: int = 0, from_self=True, deleted=False
            ...
            if line.from_self and not from_self:
                ...
                if match:
                    if not_pattern and re.search(not_pattern, line.message):
                        continue
                    if for_user and not self.server.irc_lower(line.sender) == for_user:
                        continue
                    if line.deleted and not deleted:
                        continue
    ...
    def find(
        self, pattern: typing.Union[str, typing.Pattern[str]]
