...
@utils.export(
    "serverset",
    utils.BoolSetting(
        "ctcp-responses", "Set whether I respond to CTCPs on this server"
    ),
)
class Module(ModuleManager.BaseModule):
    ...
    def ctcp_version(self, event):
        ...
        event["user"].send_ctcp_response(
            "VERSION", self.bot.config.get("ctcp-version", default)
        )

    @utils.hook("received.ctcp.request.source")
    def ctcp_source(self, event):
        event["user"].send_ctcp_response(
            "SOURCE", self.bot.config.get("ctcp-source", IRCBot.SOURCE)
        )
    ...
    def ctcp_time(self, event):
        event["user"].send_ctcp_response("TIME", datetime.datetime.now().strftime("%c"))
