...
EVENTPLAYBACK_CAP = utils.irc.Capability(
    None, "draft/event-playback", alias="event-playback"
)
...
class Module(ModuleManager.BaseModule):
    ...
    def batch_end(self, event):
        if CHATHISTORY_BATCH.match(event["batch"].type) or HISTORY_BATCH.match(
            event["batch"].type
        ):
