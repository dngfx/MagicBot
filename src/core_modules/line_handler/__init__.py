...
class Module(ModuleManager.BaseModule):
    def _handle(self, server, line):
        ...
        kwargs = {"server": server, "line": line, "direction": utils.Direction.Recv}
    ...
    def handle_raw(self, event):
        if (
            "batch" in event["line"].tags
            and event["line"].tags["batch"] in event["server"].batches
        ):
    ...
    def handle_send(self, event):
        self.events.on("raw.send").on(event["line"].command).call_unsafe(
            server=event["server"], direction=utils.Direction.Send, line=event["line"]
        )
    ...
    def handle_421(self, event):
        log.warn(
            "We sent an unknown command to %s: %s"
    ...
    def batch(self, event):
        ...
        if modifier == "+":
            ...
            self.events.on("received.batch.start").call(
