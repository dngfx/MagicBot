...
def cap(exports, events, event):
    ...
    if subcommand == "DEL":
        ...
        events.on("received.cap.del").call(
        ...
        events.on("received.cap.ack").call(
    ...
    if subcommand == "LS" or subcommand == "NEW":
        ...
        if not is_multiline:
            ...
            module_caps = events.on("received.cap.ls").call(
                capabilities=event["server"].server_capabilities, server=event["server"]
            )
    ...
    if subcommand == "ACK" or subcommand == "NAK":
        ...
        if (
            not event["server"].capabilities_requested
            and not event["server"].waiting_for_capabilities()
        ):
            event["server"].send_capability_end()


def authenticate(events, event):
    events.on("received.authenticate").call(
        message=event["line"].args[0], server=event["server"]
    )
