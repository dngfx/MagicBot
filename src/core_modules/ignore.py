...
class Module(ModuleManager.BaseModule):
    ...
    def ignore(self, event):
        ...
        if user.get_setting(setting, False):
            event["stderr"].write("I'm already ignoring '%s'%s" % (user.nickname, for_str))
        else:
            user.set_setting(setting, True)
            event["stdout"].write("Now ignoring '%s'%s" % (user.nickname, for_str))
    ...
    def unignore(self, event):
        ...
        if not user.get_setting(setting, False):
            event["stderr"].write("I'm not ignoring '%s'%s" % (user.nickname, for_str))
        else:
    ...
    def cignore(self, event):
        ...
        is_ignored = event["target"].get_user_setting(target_user.get_id(), "ignore", False)

        if remove:
            if not is_ignored:
                raise utils.EventError("I'm not ignoring %s in this channel" % target_user.nickname)
            event["target"].del_user_setting(target_user.get_id(), "ignore")
            event["stdout"].write("Unignored %s" % target_user.nickname)
        else:
            if is_ignored:
                raise utils.EventError("I'm already ignoring %s in this channel" % target_user.nickname)
    ...
    def server_ignore(self, event):
        ...
        if event["server"].get_setting(setting, False):
            event["stderr"].write("I'm already ignoring '%s' for %s" % (command, str(event["server"])))
        else:
    ...
    def server_unignore(self, event):
        ...
        if not event["server"].get_setting(setting, False):
            event["stderr"].write("I'm not ignoring '%s' for %s" % (command, str(event["server"])))
        else:
