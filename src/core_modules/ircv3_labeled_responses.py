...
CAP = utils.irc.Capability(
    "labeled-response", "draft/labeled-response-0.2", depends_on=["batch"]
)
TAG = utils.irc.MessageTag("label", "draft/label")
BATCH = utils.irc.BatchType("labeled-response", "draft/labeled-response")

CAP_TO_TAG = {"draft/labeled-response-0.2": "draft/label", "labeled-response": "label"}
...
class Module(ModuleManager.BaseModule):
    ...
    def raw_send(self, event):
        ...
        if available_cap:
            ...
            event["server"]._label_cache[label] = WaitingForLabel(
                event["line"], event["events"]
            )
    ...
    def _recv(self, server, label, lines):
        if not label in server._label_cache:
            log.debug(
                message=("unknown label received on %s: %s" % (str(server), label)),
                server=str(server),
                context="IRCv3",
                formatting=True,
            )
        ...
        cached.events.on("labeled-response").call(
            line=cached.line, responses=lines, server=server
        )
