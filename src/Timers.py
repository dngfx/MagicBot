...
class Timers(PollHook.PollHook):
    def __init__(
        self, database: Database.Database, events: EventManager.Events, log: log
    ):
    ...
    def add(
        ...
        **kwargs
        ...
        return self._add(
            None, name, delay, next_due, None, False, kwargs, callback=callback
        )

    def add_persistent(
        self, name: str, delay: float, next_due: float = None, **kwargs
    ) -> Timer:
    ...
    def _add(
        ...
        timer = Timer(id, context, name, delay, next_due, kwargs, callback=callback)
        if persist:
            self._persist(timer)
...
class TimersContext(object):
    ...
    def add(
        ...
        **kwargs
        ...
        return self._parent._add(
            self.context, name, delay, next_due, None, False, kwargs, callback=callback
        )
