...
class Servers(Table):
    def add(
        ...
        return self.database.execute_fetchone(
            "SELECT server_id FROM servers ORDER BY server_id DESC LIMIT 1"
        )[0]
    ...
    def get_all(self, enabled_only=True):
        return self.database.execute_fetchall(
            "SELECT server_id, alias FROM servers WHERE enabled=?", [int(enabled_only)]
        )
    ...
    def get_by_alias(self, alias: str) -> typing.Optional[int]:
        value = self.database.execute_fetchone(
            "SELECT server_id FROM servers WHERE alias=? COLLATE NOCASE", [alias]
        )
        if value:
            return value[0]
        return value
...
class Channels(Table):
    def add(self, server_id: int, name: str):
        ...
        return self.database.execute_fetchone(
            "SELECT channel_id FROM channels ORDER BY channel_id DESC LIMIT 1"
        )[0]
    ...
    def by_id(self, channel_id: int):
        value = self.database.execute_fetchone(
            "SELECT server_id, name FROM channels WHERE channel_id=?", [channel_id]
        )
        return value
...
class Users(Table):
    ...
    def by_id(self, user_id: int):
        return self.database.execute_fetchone(
            "SELECT server_id, nickname FROM users WHERE user_id=?", [user_id]
        )
...
class BotSettings(Table):
    ...
    def get(self, setting: str, default: typing.Any = None):
        value = self.database.execute_fetchone(
            "SELECT value FROM bot_settings WHERE setting=?", [setting.lower()]
        )
        if value:
    ...
    def delete(self, setting: str):
        self.database.execute(
            "DELETE FROM bot_settings WHERE setting=?", [setting.lower()]
        )
    ...
    def get(self, user_id: int, setting: str, default: typing.Any = None):
        value = self.database.execute_fetchone(
            """SELECT value FROM user_settings WHERE
                user_id=? and setting=?""",
            [user_id, setting.lower()],
        )
        if value:
            return json.loads(value[0])
        return default

    def find_all_by_setting(
        self, server_id: int, setting: str, default: typing.Any = []
    ):
        values = self.database.execute_fetchall(
...
class UserChannelSettings(Table):
    ...
    def get(
        self, user_id: int, channel_id: int, setting: str, default: typing.Any = None
    ):
        value = self.database.execute_fetchone(
    ...
    def find(
        self, user_id: int, channel_id: int, pattern: str, default: typing.Any = []
    ):
        values = self.database.execute_fetchall(
    ...
    def find_prefix(
        self, user_id: int, channel_id: int, prefix: str, default: typing.Any = []
    ):
...
class Database(object):
    ...
    def _execute_fetch(
        ...
        log.trace(
            self.log,
            'executed query in %fms: "%s" (params: %s)'
    ...
    def make_bot_settings_table(self):
        if not self.has_table("bot_settings"):
            ...
            self.execute(
                """CREATE INDEX bot_settings_index
    ...
    def make_user_settings_table(self):
        if not self.has_table("user_settings"):
            ...
            self.execute(
                """CREATE INDEX user_settings_index ON
