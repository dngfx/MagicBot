...
class Servers(Table):
    ...
    def by_alias(self, alias: str) -> typing.Optional[int]:
        ids = self.database.execute_fetchone("SELECT server_id FROM servers WHERE alias=?", [alias])
    ...
    def get_by_alias(self, alias: str) -> typing.Optional[int]:
        value = self.database.execute_fetchone("SELECT server_id FROM servers WHERE alias=? COLLATE NOCASE", [alias])
        if value:
            return value[0]
        return value
    ...
    def edit(self, id: int, column: str, value: typing.Any):
        ...
        self.database.execute("UPDATE servers SET %s=? WHERE server_id=?" % column, [value, id])
...
class Channels(Table):
    ...
    def add(self, server_id: int, name: str):
        self.database.execute("""INSERT OR IGNORE INTO channels
            (server_id, name) VALUES (?, ?)""",
                              [server_id,
                               name.lower()])
        return self.database.execute_fetchone("SELECT channel_id FROM channels ORDER BY channel_id DESC LIMIT 1")[0]

    def delete(self, channel_id: int):
        self.database.execute("DELETE FROM channels WHERE channel_id=?", [channel_id])
    ...
    def by_id(self, channel_id: int):
        value = self.database.execute_fetchone("SELECT server_id, name FROM channels WHERE channel_id=?", [channel_id])
        return value

    def rename(self, channel_id: int, new_name: str):
        self.database.execute("UPDATE channels SET name=? where channel_id=?", [new_name.lower(), channel_id])
...
class Users(Table):
    ...
    def delete(self, user_id: int):
        self.database.execute("DELETE FROM users WHERE user_id=?", [user_id])

    def get_id(self, server_id: int, nickname: str):
        value = self.database.execute_fetchone("SELECT user_id FROM users WHERE server_id=? and nickname=?",
                                               [server_id,
                                                nickname])
        return value if value == None else value[0]

    def by_id(self, user_id: int):
        return self.database.execute_fetchone("SELECT server_id, nickname FROM users WHERE user_id=?", [user_id])

    def get_nickname(self, server_id: int, user_id: int):
        value = self.database.execute_fetchone("SELECT nickname FROM users WHERe server_id=? AND user_id=?",
                                               [server_id,
                                                user_id])
...
class BotSettings(Table):
    ...
    def set(self, setting: str, value: typing.Any):
        self.database.execute("INSERT OR REPLACE INTO bot_settings VALUES (?, ?)", [setting.lower(), json.dumps(value)])

    def get(self, setting: str, default: typing.Any = None):
        value = self.database.execute_fetchone("SELECT value FROM bot_settings WHERE setting=?", [setting.lower()])
        if value:
            return json.loads(value[0])
        return default

    def find(self, pattern: str, default: typing.Any = []):
        values = self.database.execute_fetchall("SELECT setting, value FROM bot_settings WHERE setting LIKE ?",
                                                [pattern.lower()])
        if values:
    ...
    def delete(self, setting: str):
        self.database.execute("DELETE FROM bot_settings WHERE setting=?", [setting.lower()])


class ServerSettings(Table):

    def set(self, server_id: int, setting: str, value: typing.Any):
        self.database.execute("INSERT OR REPLACE INTO server_settings VALUES (?, ?, ?)",
                              [server_id,
                               setting.lower(),
                               json.dumps(value)])
    ...
    def delete(self, server_id: int, setting: str):
        self.database.execute("DELETE FROM server_settings WHERE server_id=? AND setting=?",
                              [server_id,
                               setting.lower()])
...
class ChannelSettings(Table):
    ...
    def set(self, channel_id: int, setting: str, value: typing.Any):
        self.database.execute("INSERT OR REPLACE INTO channel_settings VALUES (?, ?, ?)",
                              [channel_id,
                               setting.lower(),
                               json.dumps(value)])
    ...
    def find_prefix(self, channel_id: int, prefix: str, default: typing.Any = []):
        return self.find(channel_id, "%s%%" % prefix, default)

    def delete(self, channel_id: int, setting: str):
        self.database.execute("""DELETE FROM channel_settings WHERE channel_id=?
            AND setting=?""",
                              [channel_id,
                               setting.lower()])
    ...
    def find_by_setting(self, setting: str, default: typing.Any = []):
        values = self.database.execute_fetchall(
            ...
            WHERE channel_settings.setting=?""",
            [setting])
        if values:
            for i, value in enumerate(values):
...
class UserSettings(Table):
    ...
    def set(self, user_id: int, setting: str, value: typing.Any):
        self.database.execute("INSERT OR REPLACE INTO user_settings VALUES (?, ?, ?)",
                              [user_id,
                               setting.lower(),
                               json.dumps(value)])
    ...
    def find_all_by_setting(self, server_id: int, setting: str, default: typing.Any = []):
        values = self.database.execute_fetchall(
    ...
    def delete(self, user_id: int, setting: str):
        self.database.execute("""DELETE FROM user_settings WHERE
            user_id=? AND setting=?""",
                              [user_id,
                               setting.lower()])
...
class UserChannelSettings(Table):
    ...
    def set(self, user_id: int, channel_id: int, setting: str, value: typing.Any):
        self.database.execute("""INSERT OR REPLACE INTO user_channel_settings VALUES
            (?, ?, ?, ?)""",
                              [user_id,
                               channel_id,
                               setting.lower(),
    ...
    def get(self, user_id: int, channel_id: int, setting: str, default: typing.Any = None):
        value = self.database.execute_fetchone(
    ...
    def find(self, user_id: int, channel_id: int, pattern: str, default: typing.Any = []):
        values = self.database.execute_fetchall(
    ...
    def find_prefix(self, user_id: int, channel_id: int, prefix: str, default: typing.Any = []):
        return self.find(user_id, channel_id, "%s%%" % prefix, default)

    def find_by_setting(self, user_id: int, setting: str, default: typing.Any = []):
        values = self.database.execute_fetchall(
            ...
            AND user_channel_settings.user_id=?""",
            [setting,
             user_id])
        if values:
            for i, value in enumerate(values):
...
class Database(object):
    ...
    def __init__(self, log: "Logging.Log", database: str):
        ...
        self._engine.config(hostname=db_parts.hostname,
                            port=db_parts.port,
                            path=db_parts.path,
    ...
    def _execute_fetch(self,
                       query: str,
                       fetch_func: typing.Callable[[DatabaseEngineCursor],
                                                   typing.Any],
                       params: typing.List = []):
        ...
        self.log.trace("executed query in %fms: \"%s\" (params: %s)", [total_milliseconds, printable_query, params])

        return value

    def execute_fetchall(self, query: str, params: typing.List = []):
        return self._execute_fetch(query, lambda cursor: cursor.fetchall(), params)

    def execute_fetchone(self, query: str, params: typing.List = []):
        return self._execute_fetch(query, lambda cursor: cursor.fetchone(), params)
