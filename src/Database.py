...
class Servers(Table):
    ...
    def get(
            self, id: int
    ) -> typing.Tuple[int, typing.Optional[str], str, int, typing.Optional[str], bool, bool, typing.Optional[str], str,
...
class Channels(Table):
    ...
    def add(self, server_id: int, name: str):
        self.database.execute(
                ...
                (server_id, name) VALUES (?, ?)""", [server_id, name.lower()]
        )
    ...
    def get_id(self, server_id: int, name: str):
        value = self.database.execute_fetchone(
                ...
                channels WHERE server_id=? AND name=?""", [server_id, name.lower()]
        )
...
class Users(Table):
    ...
    def add(self, server_id: int, nickname: str):
        self.database.execute(
                ...
                (server_id, nickname) VALUES (?, ?)""", [server_id, nickname]
        )
    ...
    def get_id(self, server_id: int, nickname: str):
        value = self.database.execute_fetchone(
                "SELECT user_id FROM users WHERE server_id=? and nickname=?", [server_id, nickname]
        )
        return value if value == None else value[0]
    ...
    def get_nickname(self, server_id: int, user_id: int):
        value = self.database.execute_fetchone(
                "SELECT nickname FROM users WHERe server_id=? AND user_id=?", [server_id, user_id]
        )
...
class BotSettings(Table):
    ...
    def find(self, pattern: str, default: typing.Any = []):
        values = self.database.execute_fetchall(
                "SELECT setting, value FROM bot_settings WHERE setting LIKE ?", [pattern.lower()]
        )
        if values:
...
class ServerSettings(Table):
    ...
    def find(self, server_id: int, pattern: str, default: typing.Any = []):
        values = self.database.execute_fetchall(
                ...
                server_id=? AND setting LIKE ?""", [server_id, pattern.lower()]
        )
        if values:
            for i, value in enumerate(values):
                values[i] = value[0], json.loads(value[1])
    ...
    def delete(self, server_id: int, setting: str):
        self.database.execute(
                "DELETE FROM server_settings WHERE server_id=? AND setting=?", [server_id, setting.lower()]
        )
...
class UserSettings(Table):
    ...
    def find_all_by_setting(self, server_id: int, setting: str, default: typing.Any = []):
        values = self.database.execute_fetchall(
                ...
                users.server_id=? AND user_settings.setting=?""", [server_id, setting]
        )
        if values:
            for i, value in enumerate(values):
                values[i] = value[0], json.loads(value[1])
...
class UserChannelSettings(Table):
    ...
    def get(self, user_id: int, channel_id: int, setting: str, default: typing.Any = None):
        value = self.database.execute_fetchone(
                ...
                user_id=? AND channel_id=? AND setting=?""", [user_id, channel_id, setting.lower()]
        )
        if value:
            return json.loads(value[0])
        return default
...
class Database(object):
    ...
    def __init__(self, log: "log", database: str):
    ...
    def _execute_fetch(
            self, query: str, fetch_func: typing.Callable[[DatabaseEngineCursor], typing.Any], params: typing.List = []
    ):
    ...
    def make_server_settings_table(self):
        if not self.has_table("server_settings"):
            ...
            self.execute(
                    """CREATE INDEX server_settings_index
    ...
    def make_channel_settings_table(self):
        if not self.has_table("channel_settings"):
            ...
            self.execute(
                    """CREATE INDEX channel_settings_index
    ...
    def make_user_channel_settings_table(self):
        if not self.has_table("user_channel_settings"):
            ...
            self.execute(
                    """CREATE INDEX user_channel_settings_index
