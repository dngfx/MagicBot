...
class Server(IRCObject.Object):
    ...
    def find_settings(
        ...
        if not pattern == None:
            return self.bot.database.server_settings.find(self.id, pattern, default)
    ...
    def get_user_setting(
        self, nickname: str, setting: str, default: typing.Any = None
    ) -> typing.Any:
    ...
    def get_target(
        self, name: str
    ) -> typing.Optional[typing.Union[IRCChannel.Channel, IRCUser.User]]:
    ...
    def _post_read(self, lines: typing.List[str]):
        for line in lines:
            log.debug(
                message=(
                    "%s (raw recv) | %s"
                    % (str(self), IRCLine.parse_line(line).format().replace("<", "\<"))
                ),
                server=str(self),
                context="RAW RECV",
            )
            self.events.on("raw.received").call_unsafe(
                server=self, line=IRCLine.parse_line(line)
            )
            self.check_users()
    ...
    def until_next_ping(self) -> typing.Optional[float]:
        if self.ping_sent:
            return None
        return max(
            0, (self.socket.last_read + PING_INTERVAL_SECONDS) - time.monotonic()
        )
    ...
    def _send(self) -> typing.List[IRCLine.SentLine]:
        ...
        for line in lines:
            log.debug(
                message=(
                    "%s (raw send) | %s"
                    % (str(self), line.parsed_line.format().replace("<", "\<"))
                ),
                server=str(self),
                context="RAW SEND",
            )
    ...
    def send(
        ...
        self.events.on("preprocess.send").call_unsafe(
            server=self, line=line_parsed, events=line_events
        )

        if line_parsed.valid() or line_parsed.assured():
            line = line_parsed.format()
            line_obj = IRCLine.SentLine(
                line_events, datetime.datetime.utcnow(), self.hostmask(), line_parsed
            )
    ...
    def send_user(
        self, username: str, realname: str
    ) -> typing.Optional[IRCLine.SentLine]:
    ...
    def send_capability_queue(self):
        ...
        capability_batches = textwrap.wrap(
            capabilities_str, IRCLine.LINE_MAX - len("CAP REQ :")
        )
    ...
    def available_capability(
        self, capability: utils.irc.Capability
    ) -> typing.Optional[str]:
    ...
    def send_join(
        self, channel_name: str, keys: typing.List[str] = None
    ) -> typing.Optional[IRCLine.SentLine]:
        return self.send(self._line("JOIN", [channel_name] + (keys or [])))

    def send_joins(
        self, channel_names: typing.List[str], keys: typing.List[str] = None
    ):
        return self.send(self._line("JOIN", [",".join(channel_names)] + (keys or [])))

    def send_part(
        self, channel_name: str, reason: str = None
    ) -> typing.Optional[IRCLine.SentLine]:
    ...
    def send_message(
        self, target: str, message: str, tags: dict = {}
    ) -> typing.Optional[IRCLine.SentLine]:
    ...
    def send_mode(
        self, target: str, mode: str = None, args: typing.List[str] = None
    ) -> typing.Optional[IRCLine.SentLine]:
    ...
    def send_kick(
        self, channel_name: str, target: str, reason: str = None
    ) -> typing.Optional[IRCLine.SentLine]:
    ...
    def send_invite(
        self, channel_name: str, target: str
    ) -> typing.Optional[IRCLine.SentLine]:
    ...
    def send_whox(
        self, mask: str, filter: str, fields: str, label: str = None
    ) -> typing.Optional[IRCLine.SentLine]:
