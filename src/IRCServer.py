...
class Server(IRCObject.Object):
    ...
    def find_settings(self,
        ...
        if not pattern == None:
            ...
            return self.bot.database.server_settings.find_prefix(self.id, prefix, default)
        else:
    ...
    def _post_read(self, lines: typing.List[str]):
        for line in lines:
            log.debug(message=("%s (raw recv) | %s" % (str(self), IRCLine.parse_line(line).format().replace("<", "\<"))), server=str(self), context="RAW RECV")
    ...
    def _send(self) -> typing.List[IRCLine.SentLine]:
        ...
        for line in lines:
            log.debug(message=("%s (raw send) | %s" % (str(self), line.parsed_line.format().replace("<", "\<"))), server=str(self),
                      context="RAW SEND")
        return lines
    ...
    def send(self, line_parsed: IRCLine.ParsedLine, immediate: bool = False) -> typing.Optional[IRCLine.SentLine]:
        ...
        if line_parsed.valid() or line_parsed.assured():
            ...
            line_obj = IRCLine.SentLine(line_events, datetime.datetime.utcnow(), self.hostmask(), line_parsed)
    ...
    def send_capability_queue(self):
        ...
        capability_batches = textwrap.wrap(capabilities_str, IRCLine.LINE_MAX - len("CAP REQ :"))
    ...
    def send_mode(self,
                  target: str,
                  mode: str = None,
                  args: typing.List[str] = None) -> typing.Optional[IRCLine.SentLine]:
