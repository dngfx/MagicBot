...
class Channel(IRCObject.Object):
    ...
    def __init__(self, name: str, id, server: "IRCServer.Server", bot: "IRCBot.Bot"):
        ...
        self.mode_lists: typing.Dict[str,
                                     typing.Set[str]] = {}
    ...
    def mode_str(self) -> str:
        ...
        for mode, args in modes_iter:
            ...
            if (not mode in self.server.channel_list_modes and not mode in self.server.prefix_modes):
        ...
        for chunk in RE_MODES.findall(modes):
            ...
            for mode in chunk[1:]:
                ...
                if mode in self.server.channel_list_modes:
                    new_arg = args.pop(0)
                elif (mode in self.server.channel_parametered_modes or mode in self.server.prefix_modes):
                    new_arg = args.pop(0)
    ...
    def find_settings(self,
        ...
        if not pattern == None:
            return self.bot.database.channel_settings.find(self.id, pattern, default)
        elif not prefix == None:
            return self.bot.database.channel_settings.find_prefix(self.id, prefix, default)
        else:
    ...
    def find_user_settings(self,
                           ...
                           default: typing.Any = []) -> typing.List[typing.Any]:
        if not pattern == None:
            return self.bot.database.user_channel_settings.find(user_id, self.id, pattern, default)
        elif not prefix == None:
            return self.bot.database.user_channel_settings.find_prefix(user_id, self.id, prefix, default)
        else:
            raise ValueError("Please provide 'pattern' or 'prefix'")
