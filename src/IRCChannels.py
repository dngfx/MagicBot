...
class Channels(object):
    def __init__(
        self, server: "IRCServer.Server", bot: "IRCBot.Bot", events: EventManager.Events
    ):
        self._server = server
        self._bot = bot
        self._events = events
    ...
    def contains(self, name: str) -> bool:
        return (
            self._server.is_channel(name) and self._name_lower(name) in self._channels
        )
    ...
    def add(self, name: str) -> IRCChannel.Channel:
        ...
        self._events.on("new.channel").call(channel=new_channel, server=self._server)
        return new_channel
